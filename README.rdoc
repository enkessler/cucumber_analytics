...
First things first. Load up the gem code.
  ...
  require 'cucumber-analytics'

Next, let's generate a model for our Cucumber suite.

  directory = Cucumber::Analytics::ParsedDirectory.new('path/to/the/code_directory')
...
So with a few simple commands we have discoverd that there are five different
